    ...
    poll(type) {
        ...
        ping.sys.probe(self.robonectIp, async function (isAlive) {
            ...
            if (isAlive) {
                ...
                self.adapter.log.debug(type + ' polling started');
                ...
                self.adapter.log.debug('isAwake: ' + isAwake);

                if (self.batteryPollType !== 'NoPoll' && (type === 'Initial' || (self.batteryPollType === type && isAwake))) 
                    await self.pollBattery();
                if (self.errorsPollType !== 'NoPoll' && (type === 'Initial' || (self.errorsPollType === type && isAwake)))
                    await self.pollErrors();
                if (self.extensionPollType !== 'NoPoll' && (type === 'Initial' || (self.extensionPollType === type && isAwake)))
                    await self.pollExtension();
                if (self.gpsPollType !== 'NoPoll' && (type === 'Initial' || (self.gpsPollType === type && isAwake)))
                    await self.pollGPs();
                if (self.hoursPollType !== 'NoPoll' && (type === 'Initial' || (self.hoursPollType === type && isAwake)))
                    await self.pollHours();
                if (self.motorPollType !== 'NoPoll' && (type === 'Initial' || (self.motorPollType === type && isAwake)))
                    await self.pollMotor();
                if (self.portalPollType !== 'NoPoll' && (type === 'Initial' || (self.portalPollType === type && isAwake)))
                    await self.pollPortal();
                if (self.pushPollType !== 'NoPoll' && (type === 'Initial' || (self.pushPollType === type && isAwake)))
                    await self.pollPush();
                if (self.timerPollType !== 'NoPoll' && (type === 'Initial' || (self.timerPollType === type && isAwake)))
                    await self.pollTimer();
                if (self.versionPollType !== 'NoPoll' && (type === 'Initial' || (self.versionPollType === type && isAwake)))
                    await self.pollVersion();
                if (self.weatherPollType !== 'NoPoll' && (type === 'Initial' || (self.weatherPollType === type && isAwake)))
                    await self.pollWeather();
                if (self.wlanPollType !== 'NoPoll' && (type === 'Initial' || (self.wlanPollType === type && isAwake)))
                    await self.pollWlan();

                self.adapter.log.info(type + ' polling done');
            } else {
        ...
        return new Promise((resolve, reject) => {
            this.request.get({ url: apiUrl }, function (err, response, body) {
                try {
                    ...
                    if (data.successful === true) {
                        self.adapter.setState('gps.latitude', { val: convertDmsToDd(data['gps']['latitude']), ack: true });
                        self.adapter.setState('gps.longitude', { val: convertDmsToDd(data['gps']['longitude']), ack: true });
