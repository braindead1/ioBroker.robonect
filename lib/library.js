...
class Library {
    ...
    poll(pollType) {
        ...
        ping.sys.probe(self.robonectIp, async function (isAlive) {
            if (isAlive) {
                ...
                const isRestTime = self.isRestTime();
            ...
            this.request.get({ url: apiUrl }, function (err, response, body) {
                ...
                try {
                    ...
                    if (data.successful === true) {
                        const objects = require('./objects_' + cmd + '.json');
    ...
    updateExtensionStatus(ext, status) {
        ...
        this.request.get({ url: apiUrl }, function (err, response, body) {
            try {
                const data = self.parseResponse(err, response, body);
    ...
    */
    isRestTime() {
        ...
        const now = self.adapter.formatDate(new Date(), 'hh:mm');
