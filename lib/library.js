...
class Library {
        ...
        return new Promise((resolve, reject) => {
            this.request.get({ url: apiUrl }, function (err, response, body) {
                try {
                    ...
                    if (data.successful === true) {
                        self.adapter.setState('gps.latitude', { val: self.convertDdmToDd(data['gps']['latitude']), ack: true });
                        self.adapter.setState('gps.longitude', { val: self.convertDdmToDd(data['gps']['longitude']), ack: true });
                        self.adapter.setState('gps.satellites', { val: data['gps']['satellites'], ack: true });
                    } else {
                        if (data.error_message && data.error_message !== '') {
                            throw new Error(data.error_message);
                        } else {
    ...
    */
    convertDdmToDd(ddmString) {
        if (ddmString !== '') {
            let [degrees, minutes, direction] = ddmString.split(/°| /);
